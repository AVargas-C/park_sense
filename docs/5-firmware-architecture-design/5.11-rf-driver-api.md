...
STM32U585AII6Q (Application processor)
        │
        │  SPI1 (host bus) + IRQ + nRST pins
        ▼
STM32WB5MMGH6TR — M4 application core   ← runs wb_relay firmware
        │
        │  IPCC + shared SRAM             ← internal to WB5MM; transparent to U585
        ▼
STM32WB5MMGH6TR — M0+ radio core        ← runs ST Zigbee binary (firmware/zigbee_fw/)
        │
...
**Two-level architecture.** From the rf_driver's perspective, the WB5MM is an
SPI slave: U585 sends Zigbee commands over SPI1 and the WB5MM raises an IRQ
line when an event is ready. The WB5MM M4 relay firmware translates those SPI
commands to IPCC calls to the M0+ Zigbee stack — this is entirely internal to
the WB5MM chip and invisible to U585.

STM32U585 has no IPCC peripheral; IPCC is specific to multi-core STM32s
(WB/H7/MP1 families). The rf_driver on U585 uses SPI only.

The RF Driver abstracts the SPI transport into simple function calls.
